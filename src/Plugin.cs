...
[BepInPlugin("Purely_K2.CursedMultiplayer", "Cursed Word Multiplayer Mod", "1.0.0")]
public class MultiplayerManager : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        // BepInEx stores loaded plugins in BepInEx.Bootstrap.Chainloader.PluginInfos
        var multCompatableMods = BepInEx.Bootstrap.Chainloader.PluginInfos.Values
            .Where(plugin => plugin.Metadata.Name != "Multiplayer Mod")
            .ToList();
        
        Dictionary<Character, BossModifier> moddedCharacters = new Dictionary<Character, BossModifier>();
        
        foreach (var pluginInfo in multCompatableMods)
        {
            // BepInEx loaded assemblies are accessed via Instance.GetType().Assembly
            if (pluginInfo.Instance == null) continue;

            List<System.Type> modTypes = pluginInfo.Instance.GetType().Assembly.GetTypes()
            ...
            foreach (System.Type bossModifierType in bossModifiers)
            {
                ...
                Logger.LogInfo("Added \"" + bossModifierType.Name + "\" To Boss List");
            }
        }
        
        if (debugMode) Logger.LogInfo("Loaded Multiplayer Mod");
    }

    // BepInEx equivalent to OnApplicationQuit
    private void OnApplicationQuit()
    {
        ...
        if (CursedUI.lobbyName != "")
        {
            ...
            if (debugMode) Logger.LogInfo("Disconnected from lobby");
        }
        
        if (debugMode) Logger.LogInfo("Shut Down Multiplayer Mod");
    }
    #endregion
    ...
    public static class ApplyBossModifier_Patch //inBoss, total target = 1
    {
        public static void Prefix(ref List<BossModifier> ____bossModifiers)
        {
            ...
            {
                BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError(e);
            }
        }
    }
    ...
    public static class SubmitWord_Patch
    {
        ...
        public static bool Prefix(ref int ____remainingGrids, ref EncounterController __instance, ref List<TileSelection> tiles, ref List<string> words, ref GridData ____gridData, ref List<HistoricWord> ____previousWords)
        {
            ...
            try
            {
                ...
                if(____remainingGrids <= 0 && ReceivedInfo.hasOpponent)
                {
                    ...
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Finished Battle!");
                    ...
                    if(CursedUI.lobbyID != CSteamID.Nil && ReceivedInfo.opponentHealth > 0 && CursedNetworking.myPlayerPacket.health > 0 && (ReceivedInfo.opponentHighscore.Score == 0 || ReceivedInfo.opponentIsInBoss) && GameStatics.GetPlayer()?.CurrentRunProgress?.CurrentNodeType == NodeType.Boss)
                    {
                        ...
                        {
                            if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Opponent Is In Boss, waiting for them to finish...");
                            finishedFirst = true;
                        }
                    ...
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Opponent Is Done, continuing...");
                    }
                }
            }
            catch (System.Exception e)
        ...
        public static void Postfix(ref ScorePacket ____remainingTarget, ref int ____totalGridsPerRound, ref int ____remainingGrids, ref EncounterSummaryDisplayController ____encounterSummaryDisplayController)
        {
            ...
            try
            {
                if (CursedNetworking.myPlayerPacket.inBoss && ____remainingGrids > 0 && ReceivedInfo.hasOpponent)
                {
                    ...
                    else if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Not Highest Score");
                }
                else if(CursedNetworking.myPlayerPacket.highScore.Score > 0 && ReceivedInfo.opponentHighscore.Score > 0 && !ReceivedInfo.opponentIsInBoss && !CursedNetworking.myPlayerPacket.inBoss)
                {
                    if(ReceivedInfo.opponentHighscore > CursedNetworking.myPlayerPacket.highScore)
                    {
                        ...
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Lost A Life!\nCurrent Life: " + CursedNetworking.myPlayerPacket.health);
                    }
                    else if(ReceivedInfo.opponentHighscore == CursedNetworking.myPlayerPacket.highScore)
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Tied And Both Lose A Life!");
                        CursedNetworking.myPlayerPacket.UpdatePacket(CursedNetworking.myPlayerPacket.inBoss, CursedNetworking.myPlayerPacket.highScore, CursedNetworking.myPlayerPacket.health - 1);
                        ReceivedInfo.opponentHealth -= 1;
                    }
                    else
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Won The Floor!");
                        ReceivedInfo.opponentHealth -= 1;
                    }
                    ...
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Game Over! You lose!");
                        ____remainingTarget = new ScorePacket(mostRecentScorePacket.Score + ReceivedInfo.opponentHighscore.Score);
                    ...
                    {
                        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Opponent Is Scoring, Waiting...");
        ...
        public static async Task WaitForOpponentToScore()
        {
            ...
            if(ReceivedInfo.opponentHighscore.Score == 0)
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Opponent Is Done Scoring, Continuing...");
                CursedNetworking.myPlayerPacket.UpdatePacket(false, new ScorePacket(0), CursedNetworking.myPlayerPacket.health);
            }
        }
    }
    ...
    public static class Winning_Patch
    {
        public static void Postfix(ref bool __result)
        {
            if(ReceivedInfo.hasOpponent && CursedNetworking.myPlayerPacket.highScore.Score > 0)
            {
                ...
                {
                    __result = true;
                    if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("YOU WON! (Tell K2 If The Victory Screen Didn't Show Up)");
                    ReceivedInfo.ResetInfo();
                    CursedNetworking.myPlayerPacket.ResetPacket();
                }
            }
    ...
    public static class IncreaseBossReward_Patch
    {
        public static void Prefix(ref string rewardDescription, ref int rewardCashAmount)
        {
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Increasing Boss Payout");
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Must've Missed Somethin'");
            }
        }
    }
        ...
        public static bool Prefix(ref Character ____activeCharacter, ref CharacterSelectController __instance)
        {
            ...
            try
            {
                if(CursedNetworking.myPlayerPacket.myCharacterName != ____activeCharacter.GetName())
                {
                    ...
                    if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Updated Player Character to: " + ____activeCharacter.GetName());
                }
        ...
        public static void Prefix(ref ScorePacket ____remainingTarget, ref int ____remainingGrids)
        {
            ...
            if(____remainingGrids <= 0 && GameStatics.GetPlayer().CurrentRunProgress.CurrentNodeType != NodeType.Boss)
            {
                if(____remainingTarget > new ScorePacket(0L) && CursedNetworking.myPlayerPacket.health > 1)
                {
                    ...
                    if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Welp... You Lost A Life!");
                    CursedNetworking.myPlayerPacket.UpdatePacket(CursedNetworking.myPlayerPacket.inBoss, CursedNetworking.myPlayerPacket.highScore, CursedNetworking.myPlayerPacket.health - 1);
                }
            }
            if(____remainingGrids <= 0 && GameStatics.GetPlayer().CurrentRunProgress.CurrentNodeType == NodeType.Boss)
    ...
    private void Update()
    {
        ...
        if (SteamMatchmaking.GetNumLobbyMembers(CursedUI.lobbyID) > 1 && !ReceivedInfo.hasOpponent)
        {
            ...
            // Optimization: Use the built-in BepInEx plugin Logger instead of creating a new LogSource every frame
            if (debugMode) Logger.LogError("2 People In Lobby!");
        }

        if (SteamMatchmaking.GetNumLobbyMembers(CursedUI.lobbyID) == 1 && ReceivedInfo.hasOpponent)
        {
            ReceivedInfo.hasOpponent = false;
            if (debugMode) Logger.LogError("Opponent Disconnected");
    ...
    public static class HumanHitsYou_Patch
    {
        public static void Postfix()
        {
            ...
            if(ReceivedInfo.hasOpponent && encounterController != null && GameStatics.GetPlayer().CurrentRunProgress.CurrentNodeType == NodeType.Boss && encounterController.GetBossModifiers().Select(m => m.GetType()).Contains(typeof(HumanBoyBoss)) && !ReceivedInfo.noBossEffects)
            {
                ...
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Human Boy");
    ...
    public static class EnsureItGetsDoneFirst_Patch
    {
        public static void Prefix(ref EncounterSummaryDisplayController ____encounterSummaryDisplayController)
        {
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Sam");
    ...
    public static class BossPreGridStuff_Patch
    {
        public static void Prefix(ref List<BossModifier> bossModifiers)
        {
            ...
            try
            {
                ...
                {
                    if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Nat");
    ...
    public static class BossGridModifiers_Patch
    {
        ...
        public static void Prefix(ref List<BossModifier> bossModifiers, ref GridData gridData, ref List<BoardGenVizInfo> vizSteps)
        {
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Rodman");
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Hayley");
            }
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Octacles");
                        ...
                        if(tile.MyGlyphType != GlyphType.Letter)
                        {
                            switch(thisGlyphType)
                            {
                                ...
                                case GlyphType.Arrow:
                                    BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Arrow is actually used!?!" + tile.GetStringRepresentation());
                                    break;
                                default:
                                    if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Error: thisGlyphType isn't a valid glyph type");
                                    break;
                            }
                            list.Add(tile);
                        }
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Sandy");
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Beans");
    ...
    public static class BossScoreModifiers_Patch
    {
        public static void Postfix(ref ScoreCalcVizInfo __result, ref BossModifier bossModifier, ref List<TileSelection> tiles)
        {
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Nina");
                float num = 1f;
            ...
            {
                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Vs Bones");
                    ...
                    if(encounterController != null && encounterController.GetBossModifiers().Select(m => m.GetType()).Contains(typeof(NatBoss)))
                    {
                        foreach(ItemObject item in ____stampObjects)
                        {
                            if(item != null && NatBoss.unavailableItemsList.Select(i => i.GetType()).Contains(item.MyItem.GetType()))
                            {
                                ...
                                if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Found Item: " + item.MyItem.Name);
                            }
                        }
                    }
                }
    ...
    public static void SetMegMoney(long money, ref Dictionary<string, int> earningsBreakdown)
    {
        ...
        if(debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Meg's Income: " + money);
    }
...
public static class ReceivedInfo
{
    ...
    public static void SetFoeBoss(string characterName)
    {
        foreach (System.Type bossType in AllBosses)
        {
            ...
            {
                foeBoss = modifier;
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Foe Is: " + modifier.Name);
                return;
            }
        }
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Error: Couldn't Find Character");
        foeBoss = null;
    }
}
public class CursedNetworking
{
    ...
    public static void SetUpNetworking() //Called Once At Start (Includes Health Hard-Set)
    {
        ...
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Steam Linked!");
    ...
    public static void UpdateAndSendPlayerPacket(bool isLobbyOptions = false)
    {
        ...
        if(!isLobbyOptions)
        {
            ...
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Updated Info To: " + myPlayerPacket.GetAsString(':'));
        ...
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Updated Lobby Options To: " + lobbyOptions);
    }
    public static void ReceiveAndUpdateFoeInfo(LobbyDataUpdate_t callback)
    {
        if(callback.m_bSuccess == 0)
        {
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Failed To Retrieve Data For Lobby: " + callback.m_ulSteamIDLobby);
            return;
        }
        ...
        if(callback.m_ulSteamIDLobby == callback.m_ulSteamIDMember)
        {
            ...
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError(ReceivedInfo.noBossEffects + " | " + ReceivedInfo.delayScoreUpdates);
        }
        ...
        if(long.TryParse(lobbyDataList[2], out long highScoreLong) && int.TryParse(lobbyDataList[3], out int health))
        {
            if(lobbyDataList[0] != myPlayerPacket.playerName)
            {
                ...
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Received Info: " + string.Join(" | ", lobbyDataList));
                if(!ReceivedInfo.hasOpponent)
                {
                    ReceivedInfo.hasOpponent = true;
                    if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Now Have An Opponent!");
                }
            ...
            {
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Updated Info To: " + string.Join(" | ", lobbyDataList));
            }
        }
        else if(lobbyDataList.Count() == 4)
        {
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Failed To Update Player Packet Info - Ints Didn't Parse: " + string.Join(" | ", lobbyDataList));
        }
    }
}
public class CursedUI
{
    ...
    public static void TryJoinLobby()
    {
        ...
        try
        {
            ...
            if(inputLobbyCode == "")
            {
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Getting Random Lobby");
            }
    ...
    void OnLobbyMatchList(LobbyMatchList_t callback)
    {
        ...
        foreach(var lobby in listOfLobbies)
        {
            ...
            {
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Found Chosen Lobby! Joining...");
                SteamMatchmaking.JoinLobby(lobby);
                return;
            }
            else if(inputLobbyCode == "" && SteamMatchmaking.GetNumLobbyMembers(lobby) == 1)
            {
                if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Joining Random Lobby");
                SteamMatchmaking.JoinLobby(lobby);
                return;
            }
        }
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Failed To Get A Lobby To Join");
        if(inputLobbyCode == "")
    ...
    {
        if(CursedNetworking.isHost) return;
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Joined Lobby");
        ...
        if(SteamMatchmaking.GetNumLobbyMembers((CSteamID)callback.m_ulSteamIDLobby) <= 2)
        {
            ...
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Are Player 2");
        }
        else
        {
            CursedNetworking.myPlayerPacket.playerName = "Spectator";
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("You Are Spectating");
        }

        if(callback.m_EChatRoomEnterResponse != 1)
        {
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Failed To Enter Lobby: " + (uint)callback.m_EChatRoomEnterResponse);
            return;
        }
    ...
    void OnLobbyCreated(LobbyCreated_t callback)
    {
        ...
        {
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Error: Lobby Creation Failed");
            return;
        }
        ...
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Lobby Created: " + lobbyID);
    }
    ...
    public static void UpdateHearts(int myHearts = -1, int foeHearts = -1)
    {
        try
        {
            ...
            if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Health: " + myHearts + " | " + foeHearts);
...
public class NatBoss : BossModifier
{
    ...
    public List<Item> GetRandomizedUnavailableItems()
    {
        ...
        if(MultiplayerManager.debugMode) BepInEx.Logging.Logger.CreateLogSource("Multiplayer Mod").LogError("Taken Item(s): " + string.Join(" | ", unavailableItemsList));
